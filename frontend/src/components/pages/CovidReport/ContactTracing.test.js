...
test('Cancel Report', async () => {
    ...
    await act(async () => {
        const { container, getByText, getAllByText } = render(<Provider store={store}><MemoryRouter initialEntries={['/login']}>
            <RestaurantDetails alertHandler={jest.fn()}/>
        </MemoryRouter></Provider>);
        // Open report component
        fireEvent.click(getByText("Reports"))
