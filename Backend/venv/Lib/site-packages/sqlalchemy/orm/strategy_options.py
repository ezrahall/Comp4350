...
class Load(Generative, MapperOption):
    ...
    "wildcard" options that only take effect for a certain class::
        ...
        session.query(OrderSummary).options(Load(OrderSummary).lazyload('*'))

    Above, all relationships on ``OrderSummary`` will be lazy-loaded, but other
...
class _UnboundLoad(Load):
    ...
    def _bind_loader(self, entities, current_path, context, raiseerr):
        ...
        not exact API)::
            ...
            Load(
                User,
                (User, User.orders.property, OrderSummary, OrderSummary.items.property))
...
def contains_eager(loadopt, attr, alias=None):
    ...
    the desired rows, i.e.::
        ...
        sess.query(OrderSummary).\
                join(OrderSummary.user).\
                options(contains_eager(OrderSummary.user))

    The above query would join from the ``OrderSummary`` entity to its related
    ``User`` entity, and the returned ``OrderSummary`` objects would have the
    ``OrderSummary.user`` attribute pre-populated.
    ...
    should be specified using :meth:`.PropComparator.of_type`::
        ...
        sess.query(OrderSummary).\
                join((user_alias, OrderSummary.user)).\
                options(contains_eager(OrderSummary.user.of_type(user_alias)))
...
def joinedload(loadopt, attr, innerjoin=None):
    ...
    examples::
        ...
        # joined-load OrderSummary.items and then Item.keywords
        query(OrderSummary).options(
            joinedload(OrderSummary.items).joinedload(Item.keywords))

        # lazily load OrderSummary.items, but when Items are loaded,
        # joined-load the keywords collection
        query(OrderSummary).options(
            lazyload(OrderSummary.items).joinedload(Item.keywords))
    ...
    :param innerjoin: if ``True``, indicates that the joined eager load should
     use an inner join instead of the default of left outer join::
        ...
        query(OrderSummary).options(joinedload(OrderSummary.user, innerjoin=True))
...
def subqueryload(loadopt, attr):
    ...
    examples::
        ...
        # subquery-load OrderSummary.items and then Item.keywords
        query(OrderSummary).options(
            subqueryload(OrderSummary.items).subqueryload(Item.keywords))

        # lazily load OrderSummary.items, but when Items are loaded,
        # subquery-load the keywords collection
        query(OrderSummary).options(
            lazyload(OrderSummary.items).subqueryload(Item.keywords))
...
def selectinload(loadopt, attr):
    ...
    examples::
        ...
        # selectin-load OrderSummary.items and then Item.keywords
        query(OrderSummary).options(
            selectinload(OrderSummary.items).selectinload(Item.keywords))

        # lazily load OrderSummary.items, but when Items are loaded,
        # selectin-load the keywords collection
        query(OrderSummary).options(
            lazyload(OrderSummary.items).selectinload(Item.keywords))
